...
// .wrangler/tmp/bundle-cxBqH3/strip-cf-connecting-ip-header.js
...
var worker_default = {
  ...
  async fetch(req, env) {
    ...
    if (path.startsWith("/posts/")) {
      console.log("Entering /posts/ route for path:", path);
      const slug = path.slice("/posts/".length);
      const post = await loadPost(env, slug);
      console.log("Loaded post object:", post);
      if (post == null) {
        return notFound();
      }
      console.log("Post content before conversion:", post.content);
      const content = post.format === "wt" ? wikitextToHtml(post.content) : markdownToHtml(post.content);
      console.log("Content after wikitextToHtml/markdownToHtml:", content);
...
function wikitextToHtml(wt) {
  ...
  s = s.replace(/</g, "&lt;");
  s = s.replace(/>/g, "&gt;");
  s = s.replace(/'''''(.+?)'''''/g, "<em><strong>$1</strong></em>");
  s = s.replace(/'''(.+?)'''/g, "<strong>$1</strong>");
  s = s.replace(/''(.+?)''/g, "<em>$1</em>");
  s = s.replace(/<u>(.+?)<\/u>/g, "<u>$1</u>");
  s = s.replace(/<s>(.+?)<\/s>/g, "<s>$1</s>");
  s = s.replace(/<code>(.+?)<\/code>/g, "<code>$1</code>");
  s = s.replace(/<pre>(.+?)<\/pre>/g, "<pre>$1</pre>");
  s = s.replace(/\[\[(.+?)\|(.+?)\]\]/g, '<a href="$1">$2</a>');
  s = s.replace(/\[\[(.+?)\]\]/g, '<a href="$1">$1</a>');
  listBlocks.forEach((block, i) => {
    s = s.replace(`__LIST_BLOCK_${i}__`, processListBlock(block));
  });
  definitionListBlocks.forEach((block, i) => {
    s = s.replace(`__DEFINITION_LIST_BLOCK_${i}__`, processDefinitionListBlock(block));
  });
  tableBlocks.forEach((block, i) => {
    s = s.replace(`__TABLE_BLOCK_${i}__`, processTableBlock(block));
  });
  s = s.split("\n").map((line) => {
    if (line.trim() === "") {
      return "";
    }
    if (line.match(/<h\d>|<ul|<ol|<dl|<table|<pre|<code|<img|<a|<strong|<em|<u|<s/i)) {
      return line;
    }
    return `<p>${line}</p>`;
  }).join("\n");
...
// .wrangler/tmp/bundle-cxBqH3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cxBqH3/middleware-loader.entry.ts

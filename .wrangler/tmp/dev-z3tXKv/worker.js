...
// .wrangler/tmp/bundle-VR2ajE/strip-cf-connecting-ip-header.js
...
function html(s2) {
  return new Response(s2, { headers: { "content-type": "text/html; charset=utf-8" } });
}
__name(html, "html");
function notFound() {
  return new Response("Not Found", { status: 404 });
...
async function loadPost(env, slug) {
  const s2 = sanitizeSlug(slug);
  const kv = await loadPostFromKV(env, s2);
  if (kv) {
    console.log(`Loaded post from KV: ${JSON.stringify(kv)}`);
    return kv;
  }
  const md = await loadPostMarkdown(env, s2);
  if (!md)
    return null;
  const post = { slug: s2, title: s2, date: "", desc: "", format: "md", content: md };
...
function markdownToHtml(md) {
  ...
  s2 = s2.replace(/</g, "&lt;");
  s2 = s2.replace(/>/g, "&gt;");
  s2 = s2.replace(/^###\s+(.+)$/gm, "<h3>$1</h3>");
  s2 = s2.replace(/^##\s+(.+)$/gm, "<h2>$1</h2>");
  s2 = s2.replace(/^#\s+(.+)$/gm, "<h1>$1</h1>");
  s2 = s2.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>");
  s2 = s2.replace(/\*(.+?)\*/g, "<em>$1</em>");
  s2 = s2.replace(/!\[(.+?)\]\((.+?)\)/g, '<img src="$2" alt="$1">');
  s2 = s2.replace(/\[(.+?)\]\((.+?)\)/g, '<a href="$2">$1</a>');
  s2 = s2.replace(/\n{2,}/g, "\n\n");
  s2 = s2.replace(/([^\n])\n([^\n])/g, "$1<br>$2");
  s2 = s2.replace(/^(?!<h\d>|<ul>|<li>|<p>|<blockquote>|<pre>|<code>|<\/)(.+)$/gm, "<p>$1</p>");
  return s2;
}
...
function wikitextToHtml(wt) {
  return "<h1>Hello from hardcoded HTML!</h1><p>This is a test to see if wikitextToHtml is working.</p>";
...
function sanitizeSlug(s2) {
  return String(s2).toLowerCase().replace(/[^a-z0-9-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
}
...
// .wrangler/tmp/bundle-VR2ajE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VR2ajE/middleware-loader.entry.ts
